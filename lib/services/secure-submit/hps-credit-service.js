...
function HpsCreditService(hpsConfig, soapUri) {
    ...
    var list =
        function list(startDate, endDate, filterBy, callback) {
            var schema = porticoSchema.requestType('ReportActivity'),
                tx = {}, serviceName, transactionList = [], h, b, t, i;
     ...
     * @param {Number}    expMonth 1-12 padding will be handled automatically
     * @param {Number}    expYear  must be 4 digits.
     * @param {Function} callback
     *
     * @return {Object} \HpsManageTokensResponse
     */
    var updateTokenExpiration =
        function updateTokenExpiration(tokenValue, expMonth, expYear, callback) {
            var schema = porticoSchema.requestType('ManageTokens'),
                tx = {},Attribute1 = {},Attribute2 = {}, transactionRslt = {}, t;
            if (hlp.defNn(tokenValue))  {
                tx.TokenValue = tokenValue;
            }
            if (hlp.defNn(expMonth)&&hlp.defNn(expYear))  {
                tx.TokenActions = {Set: {Attribute: [{'tns:Name':'ExpMonth','tns:Value':expMonth},{'tns:Name':'ExpYear','tns:Value':expYear}]}};
            }
